...
- `polpc` as it can be seen in the table, this variable only became significant after more variables were introduced in model 2 and 3, and it has a positive slope coefficient, which indicates that crime rate per person increases with more police. Campaigning for the increase of police force may not be a good idea.

- `prbarr` became more significant, and the slope coefficient became more negative in model 2 and model 3. So the campaign might be better served on having tougher police than simply having more police 

- `prbconv` when introduced to the model, this variable was significant, and it has a negative slope, indicating the higher probability of conviction is, the lower the crime rate becomes. So we can say that higher likelihood of conviction may be a deters people from comitting crime. 
...
discuss how the significant coefficients are change
